...
import { currentPeriodStart } from "./dailyPeriod";
...
export async function recordHighScore(game, score) {
  try {
    ...
    const { data: existing } = await supabase
      ...
      .eq("game", game)
      .single();
    if (existing && existing.score >= score) return false; // only ever keep the best
...
export async function recordDailyScore(game, score) {
  try {
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const period_start = currentPeriodStart();
    const { data: existing } = await supabase
      .from("daily_scores")
      .select("score")
      .eq("user_id", user.id)
...
export function celebrateNewBest(game, score) {
  try {
    window.dispatchEvent(new CustomEvent("lyra:celebrate", { detail: { game, score } }));
  } catch (e) {
    // ignore
  }
}
